...
GO_TEST_COMMAND := go test -v -count=1 -p=10 -parallel 10 -covermode=count -coverprofile=cover.out -coverpkg ./cmd -timeout 20m
...
build-docs: ## Create the CLI mardown reference doc
	mkdir -p $(DOCS_PATH)
	cd $(DOCS_PATH) && echo "stack: docker.io/dev.local/mystack" > .appsody-config.yaml
