...
func initAppsody(stack string, template string, config *initCommandConfig) error {
	...
	if config.gitRepo != "" {
		Info.log("Initializing a git repository...")
		err = execAndWaitReturnErr("git", []string{"init"}, Debug, config.Dryrun)
		if err != nil {
			// We don't want to fail because git init failed. Just log a warning.
			Warning.log("Appsody failed to initialize the git repository.")
			// Nothing more to do. Just return from here. No need to return the error.
			return nil
		}

		Info.log("Adding a new remote called \"origin\" for the repository at the supplied url: ", config.gitRepo)
		err = execAndWaitReturnErr("git", []string{"remote", "add", "origin", config.gitRepo}, Debug, config.Dryrun)
		if err != nil {
			Warning.log("Appsody failed to add a new remote for the supplied URL")
		}
		Info.log("Adding files generated by appsody to the index")
		err = execAndWaitReturnErr("git", []string{"add", "*"}, Debug, config.Dryrun)
		if err != nil {
			Warning.log("Appsody failed to add files to the git project")
		}
	}
