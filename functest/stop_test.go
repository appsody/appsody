...
func TestStopWithoutName(t *testing.T) {
	...
	defer func() {
		...
		dockerOutput, dockerErr := cmdtest.RunCmdExec([]string{"ps", "-q", "-f", "name=" + containerName + "-dev"}, "docker", t)
...
func TestStopWithName(t *testing.T) {
	...
	defer func() {
		...
		dockerOutput, dockerErr := cmdtest.RunCmdExec([]string{"ps", "-q", "-f", "name=testStopContainer"}, "docker", t)
		if dockerErr != nil {
