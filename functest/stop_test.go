...
func TestStopWithoutName(t *testing.T) {
	...
	defer func() {
		...
		dockerOutput, dockerErr := cmdtest.RunCmdExec([]string{"ps", "-q", "-f", "name=" + containerName + "-dev"}, false, t)
...
func TestStopWithName(t *testing.T) {
	...
	defer func() {
		...
		dockerOutput, dockerErr := cmdtest.RunCmdExec([]string{"ps", "-q", "-f", "name=testStopContainer"}, false, t)
		if dockerErr != nil {
